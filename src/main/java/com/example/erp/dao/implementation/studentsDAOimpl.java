...
public class studentsDAOimpl implements studentsDAO {
    ...
    public List<Object[]> getPlaced_Students() {
        ...
        try {
            ...
            Query query = session.createQuery("select distinct concat(students.first_name,' ',students.last_name), students.roll_number, students.email, organisations.name from placement_student as ps inner join ps.stds students, placement_student as ps1 inner join ps1.plcmnt placement inner join placement.org organisations" +
                    " where students.placement_id = ps.id and ps.id = ps1.id and ps.acceptance = 'ACCEPT' and students.placement_id is not null order by students.roll_number");
    ...
    public List<Object[]> getNotPlaced_Students() {
        ...
        try {
            ...
            Query query = session.createQuery("select distinct concat(students.first_name,' ',students.last_name), s.roll_number, s.email from students s where s.placement_id is null order by s.roll_number");
